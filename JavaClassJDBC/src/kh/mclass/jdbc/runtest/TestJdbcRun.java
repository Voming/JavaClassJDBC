...
public class TestJdbcRun {
	public static void main(String[] args) {
		...
		try {
			...
			// 2. 해당 Driver 로부터 Connection instance 획득
			conn = DriverManager.getConnection("jdbc:oracle:thin:@127.0.0.1:1521:XE", // URL
					"system", // username +) 계정마다 권한도 존재함(Oracle system 이 결정) , 대소문자 구분 안함
					"oracle"); // password // 반드시 소문자 구분함

			if (conn != null) {
				System.out.println("연결 성공");

				// 3. 해당 Driver 로부터 Connection instance 획득
				stmt = conn.createStatement();

				// 4. Statement method 를 이용하여 SQL문 실행
				...
				rset = stmt.executeQuery(query); // 6. Statement method 를 이용하여 SQL문 실행
				// executeQuery : 리턴형 ResultSet , 조회/읽기(어떠한 범위 받기)
				// updateQuery : 리턴형 int, 추가/삭제/수정(성공여부:성공 1, 실패 0, 수정/삭제: 몇 개)

				while (rset.next()) { // database 는 1번부터 시작함
